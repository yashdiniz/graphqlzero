...
export const resolvers = {
  Query: {
    async users(_: undefined, args: object): Promise<Page<User>> {
      ...
      debugLog("Fetching USERs with options:", JSON.stringify(options));
  ...
  User: {
    async posts(user: User, args: object): Promise<Page<Post>> {
      const { options } = enforcePageOptionsLimit(args as { options?: PageQueryOptions });
      debugLog("Fetching POSTs for USER with id:", user.id, "and options:", JSON.stringify(options));
      return fetchUserPosts(user.id, options);
    },
    async albums(user: User, args: object): Promise<Page<Album>> {
      const { options } = enforcePageOptionsLimit(args as { options?: PageQueryOptions });
      debugLog("Fetching ALBUMs for USER with id:", user.id, "and options:", JSON.stringify(options));
      return fetchUserAlbums(user.id, options);
    },
    async todos(user: User, args: object): Promise<Page<Todo>> {
      const { options } = enforcePageOptionsLimit(args as { options?: PageQueryOptions });
      debugLog("Fetching TODOs for USER with id:", user.id, "and options:", JSON.stringify(options));
